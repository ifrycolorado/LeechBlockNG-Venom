 ...
 * file, You can obtain one at http://mozilla.org/MPL/2.0/. */
 ...
 async function fetchKeywordList() {
    try {
        const response = await fetch(browser.runtime.getURL("keywords.txt")); // Load locally
...
async function createRegExps() {
    ...
    for (let set = 1; set <= gNumSets; set++) {
        ...
        // 🚀 Inject wildcard rules directly into regex blocking
        let customWildcards = [
            "venom-blacklist",
        ...
        // 🚀 Inject keyword-based blocking from file
        let blockedKeywords = await fetchKeywordList();
        let keywordRegex = blockedKeywords.map(word => `.*${word}.*`).join("|");

        let existingBlockRegex = gRegExps[set].block ? gRegExps[set].block.source : "";
        let newBlockRegex = [...new Set([existingBlockRegex, ...customWildcards, keywordRegex].filter(Boolean))].join("|");

        gRegExps[set].block = new RegExp(newBlockRegex, "i");
    }

    console.log("✅ Injected wildcard & keyword rules into createRegExps.");
}
 ...
 function testURL(url, referrer, blockRE, allowRE, referRE, allowRefers) {
    let block = blockRE ? blockRE.test(url) : false; // ✅ Always return true/false
    let allow = allowRE ? allowRE.test(url) : false;
    let refer = referRE ? referRE.test(referrer) : false;
 ...
 function checkTab(id, isBeforeNav, isRepeat) {
	 ...
	 for (let set = 1; set <= gNumSets; set++) {
		 ...
		 if (testURL(pageURL, referrer, blockRE, allowRE, referRE, allowRefers)
			 ...
			 // 🚀 Force wildcard blocks to be 24/7
			 if (blockRE && blockRE.source.includes("venom-blacklist")) {
				console.log("✅ Enforcing 24/7 blocking for wildcard sites:", pageURL);
				days = [true, true, true, true, true, true, true]; // ✅ Every day
				times = "0000-2400"; // ✅ All day
				activeBlock = true; // ✅ Always enabled
			}
			...
			// 🚀 Ensure wildcard sites always trigger `doBlock`
			if (blockRE && blockRE.source.includes("venom-blacklist")) {
				doBlock = true;
			}
			 ...
			 if (!override && doBlock && (!isRepeat || activeBlock || blockRE.source.includes("venom-blacklist"))) {
    			...
    			blockURL = browser.runtime.getURL("blocked.html");
				 ...
				 function applyBlock(keyword) {
					console.log("🚫 applyBlock() executing for:", pageURL, "| Keyword:", keyword);
 ...
 function updateTimeData(id, secsOpen, secsFocus) {
	 ...
	 for (let set = 1; set <= gNumSets; set++) {
		 ...
		 let block = false; // ✅ Ensure block is always defined
		 let allow = false;
		 let refer = false;
